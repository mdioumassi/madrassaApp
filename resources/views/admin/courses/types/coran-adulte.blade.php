...
@section('content')
    <div class="container">
        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header text-uppercase  w3-light-blue">{{ __('Cours de coran pour adulte') }}</div>
                    <div class="card-body">
                        <ul class="nav nav-tabs">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active w3-light-blue"
                                    href="{{ route('admin.courses.select.levels.keyword', 'coran-adulte') }}">{{ _('Cours de coran pour adulte') }}</a>
                            </li>
                        </ul>
                        <button class="w3-button w3-xlarge w3-circle w3-light-blue w3-card-4 w3-margin"data-bs-toggle="modal"
                        data-bs-target="#modal-create-course-level" >+</button>
                        <table class="table mt-2">
                            <thead>
                                <tr>
                                    <th class="w3-light-blue text-light">{{ _('Libelle') }}</th>
                                    <th class="w3-light-blue text-light">{{ _('Tarif') }}</th>
                                    <th class="w3-light-blue text-light">{{ _('Frais d\'inscription') }}</th>
                                    <th class="w3-light-blue text-light">{{ _('Horaires') }}</th>
                                    <th class="w3-light-blue text-light">{{ _('Matières') }}</th>
                                    <th class="w3-light-blue text-light">{{ _('Professeur') }}</th>
                                    <th class="w3-light-blue  text-light">{{ _('Actions') }}</th>
                                </tr>
                            </thead>
                            @if ($levels->count() == 0)
                                <tr>
